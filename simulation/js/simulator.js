...
$("#imageY").on("dragstart", function (e) {
    e.originalEvent.dataTransfer.setData("text/plain", "dragging-y"); // Allow image Y to be draggable
});

$("#imageA").on("dragstart", function (e) {
    e.originalEvent.dataTransfer.setData("text/plain", "dragging-a"); // Allow image A to be draggable
});

$("#imageB").on("dragstart", function (e) {
    e.originalEvent.dataTransfer.setData("text/plain", "dragging-b"); // Allow image B to be draggable
});

$("#imageX").on("dragover", function (e) {
    e.preventDefault();
});

$("#imageX").on("drop", function (e) {
    e.preventDefault();
    const draggedItem = e.originalEvent.dataTransfer.getData("text/plain");

    if (draggedItem === "dragging-y" && !isImageYDropped) {
        $("#imageY").css("visibility", "hidden");
        isImageYDropped = true; // Set the flag when image-_ is dropped
        $("#imageX").attr("src", "../images/parts/sh-1.png"); // Replace image X with image Z when any image is dropped onto it
        $("#insertButton").prop("disabled", false);
    }
    else if (draggedItem === "dragging-a" && !isImageYDropped) {
        $("#imageA").css("visibility", "hidden");
        isImageYDropped = true; // Set the flag when image-_ is dropped

        $("#imageX").attr("src", "../images/parts/sh-2.png"); // Replace image X with image Z when any image is dropped onto it
        $("#insertButton").prop("disabled", false);
    }
    else if (draggedItem === "dragging-b" && !isImageYDropped) {
        $("#imageB").css("visibility", "hidden");
        isImageYDropped = true; // Set the flag when image-_ is dropped

        $("#imageX").attr("src", "../images/parts/sh-3.png"); // Replace image X with image Z when any image is dropped onto it
        $("#insertButton").prop("disabled", false);
    }
});
